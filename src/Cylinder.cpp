...
void Cylinder::init(float radius, float height, int segments) {
    ...
    for (int i = 0; i <= segments; i++) {
        ...
        // normal lateral é perpendicular ao eixo Y
        float nx = cos(theta);
        float nz = sin(theta);

        // topo
        vertices.insert(vertices.end(), {x, halfH, z, nx, 0.0f, nz, u, 1.0f});
        // base
        vertices.insert(vertices.end(), {x, -halfH, z, nx, 0.0f, nz, u, 0.0f});
    }
